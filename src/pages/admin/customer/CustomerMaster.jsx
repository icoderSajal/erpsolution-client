...
const CustomerMaster = () => {
  ...
  const handleToggleChange = (e) => {
    ...
    setMenu((prev) => ({ ...prev, active: value }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        const res = await api.put(`/admin/update-customer/${editId}`, menu, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.success) {
          toast.success("Cutomer updated successfully");
          fetchData();
        }
      } else {
        const res = await api.post("/admin/create-customer", menu, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const res = await api.put(`/admin/delete-customer/${id}`, {
        headers: {
  ...
  const openModal = (menu = null) => {
    if (menu) {
      setMenu({
        ...
        customerDescription: menu.customerDescription,
        active: Number(menu.active),
      });
      setEditId(menu._id);
    } else {
      setMenu({ customerName: "", customerDescription: "", active: 0 });
      setEditId(null);
    }
    setIsModalOpen(true);
  };
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.data.success) {
        setMenus(response.data.customers);
        setFilteredMenus(response.data.customers);
      }
    } catch (error) {
            ...
            <table className="w-full text-sm text-left text-gray-700">
              <thead className="text-xs uppercase bg-gray-100 text-gray-800">
                <tr>
                  ...
                  <th className="px-6 py-4">Cutomer Profile Name</th>
              ...
              <tbody>
                {currentMenus.map((menu, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{menu.customerName}</td>
                    <td className="px-6 py-4">{menu.customerDescription}</td>
                    <td className="px-6 py-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    name="customerName"
                    value={menu.customerName}
                    onChange={handleChange}
                    placeholder="e.g., Customer Profile name"
                ...
                <div>
                  ...
                  <input
                    ...
                    value={menu.customerDescription}
                    onChange={handleChange}
                    placeholder="e.g., Customer Description"
                    className="w-full border border-gray-300 rounded px-3 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-gray-500"
                  />
                </div>
