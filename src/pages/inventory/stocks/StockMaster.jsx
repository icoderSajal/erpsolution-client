...
export default function StockMaster() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        const res = await api.put(`/inventory/update-unit/${editId}`, menu, {
        ...
        const res = await api.post(`/inventory/create-unit`, menu, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const res = await api.put(`/inventory/delete-unit/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      if (res.data.success) {
        toast.success("Deleted Unit");
      }
    } catch (error) {
  ...
  const openModal = (menu = null) => {
    if (menu) {
      setMenu({
        ...
        unitValue: menu.unitValue,
        active: Number(menu.active),
      });
      setEditId(menu._id);
    } else {
      setMenu({ unitName: "", unitValue: "", active: 0 });
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.data.success) {
        setMenus(response.data.units);
        setFilteredMenus(response.data.units);
      }
    } catch (error) {
            ...
            <table className="w-full text-sm text-left text-gray-700">
              <thead className="text-xs uppercase bg-gray-100 text-gray-800">
                <tr>
                  ...
                  <th className="px-6 py-4">Out of Stock</th>
              ...
              <tbody>
                {currentMenus.map((menu, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{menu.unitName}</td>
                    <td className="px-6 py-4">{menu.unitValue}</td>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="unitValue"
                    value={menu.unitValue}
                    onChange={handleChange}
                    placeholder="e.g., Unit Value"
                    className="w-full border border-gray-300 rounded px-3 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-gray-500"
                  />
                </div>
