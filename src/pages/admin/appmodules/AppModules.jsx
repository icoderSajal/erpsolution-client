...
const AppModules = () => {
  ...
  const openModal = (menu = null) => {
    if (menu) {
      ...
      setMenu({ moduleName: "", modulePath: "", active: 0 });
      setEditId(null);
    }
    setIsModalOpen(true);
  };
  ...
  const handleEndpointChange = (e) => {
    ...
    setMenu((prev) => ({ ...prev, modulePath: value }));
  };

  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const res = await api.delete(`/admin/delete-appmodule/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      if (editId) {
        const res = await api.put(`/admin/update-appmodule/${editId}`, menu, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.success) {
        ...
        const res = await api.post(`/admin/create-appmodule`, menu, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.success) {
      ...
      closeModal(); // ✅ now will close after successful save/update
    } catch (error) {
      const errMsg = error.res?.data?.msg || "Action failed. Try again.";
      toast.error(errMsg);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-gray-600 text-white px-5 py-2 rounded hover:ring-1 hover:bg-black transition-all"
            >
              Back
            </button>
          </div>
            ...
            <table className="w-full text-sm text-left text-gray-700">
              <thead className="text-xs uppercase bg-gray-100 text-gray-800">
                <tr>
                  ...
                  <th className="px-6 py-4">Permission Status</th>
              ...
              <tbody>
                {currentMenus.map((menu, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{menu.moduleName}</td>
                    <td className="px-6 py-4">{menu.modulePath}</td>
                    <td className="px-6 py-4">
          ...
          >
            ...
            >
              <h2 className="text-xl font-bold text-center text-gray-800 mb-4">
                {editId ? "Edit Modules" : "Add Modules"}
              </h2>
