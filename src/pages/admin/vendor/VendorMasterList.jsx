...
const VendorMasterList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get("/admin/getvendors", {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      if (response.data.success) {
        setMenus(response.data.vendors);
        setFilteredMenus(response.data.vendors);
      }
    } catch {
              ...
              <tbody>
                {currentMenus.map((menu, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3">{menu.vendorName}</td>
                    ...
                    <td className="px-6 py-4 text-center space-x-3">
                      <button
                        ...
                        onClick={() =>
                          navigate(`/admin/edit-vendor/${menu._id}`)
                        }
                      >
                        <FaEdit />
                      </button>
