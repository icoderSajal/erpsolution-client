...
export default function UserSummary() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/user/route-permissions", {
        headers: {
      ...
      if (res.data.success) {
        ...
        const userRoutes = res.data.routes.filter(
          (route) => route.moduleId?.moduleName === "MyHR"
        );
    ...
    <>
      <div className="p-6 max-w-6xl mx-auto text-white bg-gray-600 rounded-2xl">
        <div className=" flex justify-between items-center">
          <h1 className="text-2xl font-semibold mb-5 text-left">MyHR Menus</h1>
          <NavLink
            to="/"
