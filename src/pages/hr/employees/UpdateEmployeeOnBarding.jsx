...
export default function UpdateEmployeeOnBarding() {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      try {
        const response = await api.get(`/hradmin/get-userById/${id}`, {
          headers: {
        ...
        if (response.data.success) {
          ...
          //alert(JSON.stringify(employee.doj));
          setFormData((prev) => ({
            ...prev,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              name="doj"
              value={new Date(formData.doj).toLocaleDateString()}
              onChange={handleChange}
              className="mt-1 p-2 block w-full border border-gray-300 rounded-md"
            />
            {formErrors.doj && (
