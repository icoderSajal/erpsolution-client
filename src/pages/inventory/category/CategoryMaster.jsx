...
export default function CategoryMaster() {
    ...
    try {
      ...
      if (editId) {
        const res = await api.put(
          `/inventory/update-category/${editId}`,
          menu,
          {
        ...
        const res = await api.post("/inventory/create-category/", menu, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const res = await api.put(`/inventory/delete-category/${id}`, {
        headers: {
  ...
  const openModal = (menu = null) => {
    if (menu) {
      setMenu({
        catgoryCode: menu.catgoryCode,
        catgoryName: menu.catgoryName,
        active: Number(menu.active),
      });
      setEditId(menu._id);
    } else {
      setMenu({ catgoryCode: "", catgoryName: "", active: 0 });
  ...
  const closeModal = () => {
    ...
    setEditId(null);
    setMenu({ catgoryCode: "", catgoryName: 0 });
  };
  ...
  const fetchData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setFilteredMenus(response.data.categories);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (search.trim() === "") {
      ...
      setFilteredMenus(
        menus.filter((m) =>
          m.catgoryCode.toLowerCase().includes(search.toLowerCase())
        )
      );
      setCurrentPage(1);
    }
              ...
              <tbody>
                {currentMenus.map((menu, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{menu.catgoryCode}</td>
                    <td className="px-6 py-4">{menu.catgoryName}</td>
                    <td className="px-6 py-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    name="catgoryCode"
                    maxLength={3}
                    value={menu.catgoryCode}
                    onChange={handleChange}
                    placeholder="e.g., Category Code "
                    className="w-full border uppercase border-gray-300 rounded px-3 py-2 mt-1 focus:outline-none focus:ring-2 focus:ring-gray-500"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
